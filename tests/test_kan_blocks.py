...
def test_kan_conv1d_shape(basis):
    conv = KANConv1d(
        in_channels=3, out_channels=6, kernel_size=3, basis=basis, key=_ks(1)[0]
    )
...
def test_kan_conv3d_shape():
    conv = KANConv3d(
        in_channels=2, out_channels=4, kernel_size=3, basis="rbf", key=_ks(1)[0]
    )
...
def test_kan_conv3d_grad():
    conv = KANConv3d(
        in_channels=2, out_channels=2, kernel_size=3, basis="chebyshev", key=_ks(1)[0]
    )
...
def test_kan_spectral_block2d_pipe_jit():
    p = fx.block(
        KANSpectralBlock2d(2, 16, n_modes=4, basis="rbf", key=_ks(2)[0])
...
def test_kan_res_block_actually_residual():
    ...
    blk = eqx.tree_at(
        lambda m: (m.kan1.spline_weight, m.kan2.spline_weight),
        blk,
        (
            jnp.zeros_like(blk.kan1.spline_weight),
...
def test_kan_res_block_with_layer_norm():
    blk = KANResBlock(features=8, basis="rbf", use_layer_norm=True, key=_ks(1)[0])
    y = blk(jnp.ones((5, 8)))
    assert y.shape == (5, 8)
...
def test_kan_attention_block_shape():
    blk = KANAttentionBlock(features=16, num_heads=4, basis="rbf", key=_ks(1)[0])
    y = blk(jnp.ones((10, 16)))
    assert y.shape == (10, 16)


def test_kan_attention_block_jit():
    blk = KANAttentionBlock(features=16, num_heads=4, basis="chebyshev", key=_ks(1)[0])
