...
from foundax.architectures.kan import (
    JacobiBasis, LegendreBasis,
    JacobiKANLayer, LegendreKANLayer,
    ChebyshevBasis,
    BSRBFBasis, BSplineBasis, RBFBasis,
    KAN, EfficientKAN,
)
...
def test_bsrbf_basis_concat_matches_components():
    ...
    hyb = BSRBFBasis(in_features=1, grid_size=grid_size, spline_order=k,
                     rbf_grid_size=rbf_size)
...
def test_kan_and_efficient_kan_have_same_forward_signature():
    ...
    key = _k()
    kw = dict(in_features=3, output_dim=2, hidden_dims=8, num_layers=2,
...
def test_block_is_transparent_for_all_variants():
    ...
    LAYERS = [
        kan_mod.KANLayer, kan_mod.EfficientKANLayer, kan_mod.FastKANLayer,
        kan_mod.FourierKANLayer, kan_mod.ChebyshevKANLayer,
        kan_mod.JacobiKANLayer, kan_mod.LegendreKANLayer,
        kan_mod.WaveletKANLayer, kan_mod.TaylorKANLayer,
        kan_mod.HermiteKANLayer, kan_mod.LaguerreKANLayer,
        kan_mod.BernsteinKANLayer, kan_mod.ReLUKANLayer,
        kan_mod.RationalKANLayer, kan_mod.SincKANLayer,
        kan_mod.GramKANLayer, kan_mod.BSRBFKANLayer,
    ]
    for cls in LAYERS:
