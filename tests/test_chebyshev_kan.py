"""Tests for ChebyshevKANLayer + ChebyshevKAN."""
...
from foundax.architectures.kan import ChebyshevKANLayer, ChebyshevBasis
...
LAYER = lambda i, o, **kw: ChebyshevKANLayer(i, o, degree=5, key=kw["key"])
...
def test_network_train():
    network_train_overfit_sin(lambda **kw: fx.chebyshev_kan(**kw, degree=8))


def test_chebyshev_matches_numpy_polynomial():
    """Chebyshev T_n basis must equal numpy.polynomial.chebyshev evaluation."""
    from numpy.polynomial import chebyshev as Tcheb

    basis = ChebyshevBasis(degree=6)
    x = jnp.linspace(-0.9, 0.9, 25).reshape(-1, 1)
    phi = np.asarray(basis(x))[:, 0, :]  # (25, 7)
    # input is squashed by tanh inside the basis
    xn = np.tanh(np.asarray(x)[:, 0])
    ref = np.zeros((xn.shape[0], 7))
    for n in range(7):
        coeffs = np.zeros(n + 1); coeffs[n] = 1.0
        ref[:, n] = Tcheb.chebval(xn, coeffs)
