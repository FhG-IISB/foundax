...
def test_block_custom_name():
    b = fx.block(fx.layers.SpectralBlock2d(3, 32, n_modes=8, key=_ks(1)[0]), name="encoder")
...
def test_block_standalone_model():
    ...
    fno = fx.fno2d(in_features=3, hidden_channels=16, n_modes=4, d_vars=1, key=_ks(1)[0])
    x = jnp.ones((8, 8, 3))
    y = fno(x)
    assert y.shape[2] == 1
