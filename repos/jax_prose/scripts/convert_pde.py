...
def main():
    ap = argparse.ArgumentParser(
        description="Convert PROSE-PDE checkpoint to JAX msgpack"
    )
    ...
    for k, v in state.items():
        ...
        if k.startswith("data_decoder."):
            ...
            if rest == "proj.0.weight":
                _set_param(
                    params, ["data_decoder", "proj_0", "kernel"], jnp.asarray(a.T)
                )
                handled.add(k)
                continue
            if rest == "proj.0.bias":
            ...
            if rest == "proj.1.weight":
                _set_param(
                    params, ["data_decoder", "proj_1", "kernel"], jnp.asarray(a.T)
                )
                handled.add(k)
                continue
            if rest == "proj.1.bias":
