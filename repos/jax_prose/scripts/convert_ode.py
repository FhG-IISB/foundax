...
from jax_prose.prose_ode_pde_2to1 import PROSEODE2to1, ProseTextData2to1Config
...
def main():
    ...
    ap.add_argument("--max-output-dimension", type=int, default=3)
    ap.add_argument("--input-len", type=int, default=50)
    ap.add_argument("--output-len", type=int, default=50)
    ...
    x = jnp.ones((args.input_len, 1, 1 + args.max_output_dimension), dtype=jnp.float32)
    ...
    for k, v in state.items():
        ...
        if k.startswith("data_decoder."):
            ...
            if _map_ffn(rest, ["data_decoder"], params, a):
                ...
                continue
            if rest == "proj.weight":
                _set_param(params, ["data_decoder", "proj", "kernel"], jnp.asarray(a.T))
                handled.add(k)
                continue
            if rest == "proj.bias":
                _set_param(params, ["data_decoder", "proj", "bias"], jnp.asarray(a))
                handled.add(k)
                continue
