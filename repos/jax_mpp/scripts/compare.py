...
Without --checkpoint: structural validation with random weights (JAX only).
With --checkpoint:    full numerical comparison against PyTorch.

Usage (structural check, no original repo needed):
    python compare.py --variant Ti

Usage (full numerical comparison):
    PYTHONPATH=/path/to/multiple_physics_pretraining python compare.py \\
        --variant Ti --checkpoint /path/to/MPP_AViT_Ti.pt
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint", type=str, default=None,
                        help="PyTorch checkpoint path (omit for structural check)")
...
def run_full_comparison(args: argparse.Namespace) -> int:
    ...
    cfg = AVIT_CONFIGS[args.variant]
    print(f"\n[MPP-AViT-{args.variant}] Full numerical comparison")
    ...
    try:
        ...
        print("Set PYTHONPATH=/path/to/multiple_physics_pretraining")
        return 1
    ...
    jax_model = eqx.tree_at(lambda m: m, jax_model, replace_fn=lambda _: jax_params)
